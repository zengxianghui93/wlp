	...
	$(function(){
		...
		for(var i=0;i<"${pagedNews.totalPageCount}"*1;i++){
			if((i+1) == "${pagedNews.currentPageNo}"){
				options += '<option selected="selected" value="${context}/manage/statisticalAnalysis/toJournal.html?pageNo='+(i+1)+'">'+(i+1)+'</option>';
			}else{
				options += '<option value="${context}/manage/statisticalAnalysis/toJournal.html?pageNo='+(i+1)+'">'+(i+1)+'</option>';
			}
		}
...
<body>
    <div class="YJ2016_HTmain YJ2016_clearfix " >
        ...
        <div class="con" style="float: left">
        	<div class="HTc1">
        	<span style="margin: 2%">中文期刊内容统计:</span>
            <table id="table1" style="margin-top: 1%;text-align: center;">
               ...
               <c:forEach var="res" items="${pagedNews.result}">    
				    <tr>
			        	...
			        	<td class="ct3"><a href="${context}/manage/statisticalAnalysis/journalDetail.html?name=${res.name}&pageNo=${pagedNews.currentPageNo}" >查看</a></td>
					</tr>
				</c:forEach>
            </table>
            <div class="notePage">
            <div class="page">
                   	...
                   	<c:if test="${pagedNews.currentPageNo >1 }">
						<a href="${context}/manage/statisticalAnalysis/toJournal.html?pageNo=1" style="margin-right:5px;">首页</a>
					</c:if>
                   	
                   	<c:if test="${!pagedNews.hasPreviousPage}">上一页</c:if>
					<c:if test="${pagedNews.hasPreviousPage}">
				    	  <a href="${context}/manage/statisticalAnalysis/toJournal.html?pageNo=${pagedNews.currentPageNo -1 }" style="margin-right:5px;">上一页</a>
				    </c:if>
				    
					<c:if test="${!pagedNews.hasNextPage}">下一页</c:if>
					<c:if test="${pagedNews.hasNextPage}">
				   		<a href="${context}/manage/statisticalAnalysis/toJournal.html?pageNo=${pagedNews.currentPageNo +1 }" style="margin-right:5px;">下一页</a>
				    </c:if>
					...
					<c:if test="${pagedNews.currentPageNo < pagedNews.totalPageCount}">
						<a href="${context}/manage/statisticalAnalysis/toJournal.html?pageNo=${pagedNews.totalPageCount }" style="margin-right:5px;">尾页</a>
					</c:if>
                    
                   	 第
                    <select onchange="self.location.href= options[selectedIndex].value" id="currentPageNoSelect">
                    	<option value="${context}/manage/statisticalAnalysis/toJournal.html?pageNo=${pagedNews.currentPageNo}.html" >${pagedNews.currentPageNo}</option>
                    </select>页              
            </div></div>
            </div>
        </div><!--con-->
