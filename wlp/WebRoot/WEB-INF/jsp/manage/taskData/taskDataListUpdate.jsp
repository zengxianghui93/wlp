...
$(function(){
	...
	<c:forEach items="${resChecks}" var="item" varStatus="status">
		if("${taskData.taskTableName}"=="${item.resTblName}"){
			checkUsers += '<option value="${item.resTblName}" selected="selected">${item.resName}</option>';
		}else{
	...
	<c:forEach items="${resAll}" var="item" varStatus="status">
		if("${taskData.taskTableName}"=="${item.resTblName}"){
			updateUsers += '<option value="${item.resTblName}" selected="selected">${item.resName}</option>';
		}else{
	...
	<c:forEach items="${taskCheckUsers}" var="item" varStatus="status">
		if("${taskData.taskUserId}"=="${item.userId}"){
			taskCheckUsers += '<option value="${item.userId}" selected="selected">${item.userName}</option>';
		}else{
	...
	<c:forEach items="${taskUpdateUsers}" var="item" varStatus="status">
		if("${taskData.taskUserId}"=="${item.userId}"){
			taskUpdateUsers += '<option value="${item.userId}" selected="selected">${item.userName}</option>';
		}else{
	...
	//初始化资源库
	var obj4 = document.getElementById("taskType"); 
	selTableName(obj4.options[obj4.selectedIndex].value);
	...
	<div class="YJ2016_HTmain YJ2016_clearfix ">
		...
		<div class="con" align="center"  style="float: left;margin-left: 4%">
			<div class="HTnd1" style="display: inline-block;" style="margin-right: 100%;">
            	<span class="setTit">数据任务修改</span>
                <div class="HTnd_gdmod">
                <form name="form1" action="${context}/manage/taskData/taskDataUpdate.html"
					method="post" enctype="multipart/form-data">
					<input type="hidden" value="${taskData.id}" name="id"/>
					<input type="hidden" value="${taskData.creatTime}" name="creatTime2"/>
					<input type="hidden" value="${taskData.taskResIds}" name="ids"/>
					<input type="hidden" value="${taskData.resBegin}" name="begin"/>
					<input type="hidden" value="${taskData.resEnd}" name="end"/>
               		<p><span style="margin-left: -5%;width:20%">任务名称：</span><input type="text" name="taskName" id="taskName" value="${taskData.taskName}" /></p>
               		<p><span style="margin-left: -5%;width:20%">任务类型：</span>
               			<select name="taskType" id="taskType" style="width: 420px" onchange="selTableName(options[selectedIndex].value)">
							<c:forEach items="${taskTypes}" var="item" varStatus="status">
								<option value="${item.typeId}" <c:if test="${item.typeId==taskData.taskType}">selected="selected"</c:if>>${item.typeName}</option>
							</c:forEach> 
						</select></p>
                	<p><span style="margin-left: -5%;width:20%">任务描述：</span><textarea name="taskDesc" cols="57" rows="5" >${taskData.taskDesc}</textarea></p>
                	...
                	<p><span style="margin-left: -5%;width:20%">方式：</span><select style="width: 420px" name="taskWay" id="taskWay" onchange="selTypeType(options[selectedIndex].value)">
                			<option value="1" <c:if test="${taskData.taskWay=='1'}">selected="selected"</c:if>>范围</option>
                			<option value="2" <c:if test="${taskData.taskWay=='2'}">selected="selected"</c:if>>数量</option>
                		</select></p>
                	<p id="p1"><span style="margin-left: -5%;width:20%">采集类型：</span><select style="width: 420px"  name="taskIsGather" id="taskIsGather" onchange="getNoCheckNum()">
                			<option value="" <c:if test="${taskData.taskIsGather==null||taskData.taskIsGather==''}">selected="selected"</c:if>>所有数据</option>
                			<option value="0" <c:if test="${taskData.taskIsGather=='0'}">selected="selected"</c:if>>采集数据</option>
                			<option value="1" <c:if test="${taskData.taskIsGather=='1'}">selected="selected"</c:if>>本地数据</option>
                		</select></p>
                	<p id="p2"><span style="margin-left: -5%;width:20%"><select name="taskResFldName" id="taskResFldName"><option>标题</option><option>正文</option></select></span><input name="taskResKeyword" id="taskResKeyword" onblur="getNoCheckNum()" value="${taskData.taskResKeyword}"/></p>
                	<p id="p3"><span style="margin-left: -5%;width:20%">数量：</span><input type="text" style="width: 307px" name="taskResNums" id="taskResNums" onkeyup="isNum()" value="${taskData.taskResNums}"/><span id="allNums" style="width:100px">(总数:统计中)</span></p>
                	...
                	<p id="p4"><span style="margin-left: -5%;width:20%">id范围：</span><input style="width: 185px" type="text" name="resBegin" id="resBegin" value="${taskData.resBegin}"/>&nbsp;至&nbsp;<input style="width: 185px" name="resEnd" type="text" id="resEnd" value="${taskData.resEnd}"/></p>
                	<p><span style="margin-left: -5%;width:20%">处理时间：</span><input style="width: 185px" type="text" onClick="WdatePicker()" class="Wdate" name="handBeginTime2" id="handBeginTime2" value="<fmt:formatDate value="${taskData.handBeginTime}" type="date" dateStyle="default"/>"/>&nbsp;至&nbsp;<input style="width: 185px" name="handEndTime2" id="handEndTime2" type="text" value="<fmt:formatDate value="${taskData.handEndTime}" type="date" dateStyle="default"/>" onClick="WdatePicker()" class="Wdate"/></p>
