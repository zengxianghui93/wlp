...
<body>
	...
	<div class="YJ2016_HTmain YJ2016_clearfix ">
		...
		<form name="form" action="${context}/manage/onesleect.html" method="post">
			...
			<div class="userbtn">
				 <a href="${context}/manage/chongzhi.html" class="userfresh">刷新</a>
			</div>
			<table class="userTable" style="margin-top: 20px;">
				<tr class="cTitle">
					<td style="width: 5%;">序号</td>
					<td style="width: 5%;">用户名</td>
					<td style="width: 10%;">卡内余额</td>
				...
				<c:forEach var="user" items="${pagedNews.result}" varStatus="status">
					<tr>
						...
						<td><a href="${context}/manage/record-${user.id}.html">${user.user_name}</a></td>
						<td>${user.wf_balance}</td>
						<td><a href="${context}/manage/chgonziaccount-${user.id}.html" style="color:#3575b5;">充值</a></td>
			...
			<div class="notePage">
				<div class="page">
					...
					<c:if test="${pagedNews.currentPageNo >1 }">
						<a href="${context}/manage/onesleectaccounts-${name}.html?pageNo=1"
					...
					<c:if test="${pagedNews.hasPreviousPage}">
						<a
							href="${context}/manage/onesleectaccounts-${name}.html?pageNo=${pagedNews.currentPageNo -1 }"
					...
					<c:if test="${pagedNews.currentPageNo < pagedNews.totalPageCount}">
						<a
							href="${context}/manage/onesleectaccounts-${name}.html?pageNo=${pagedNews.totalPageCount }"
					...
					第 <select
						...
						<option
							value="${context}/manage/onesleectaccounts-${name}.html?pageNo=${pagedNews.currentPageNo}.html">${pagedNews.currentPageNo}</option>
						<c:if
							test="${pagedNews.currentPageNo+1 <= pagedNews.totalPageCount}">
							<option
								value="${context}/manage/onesleectaccounts-${name}.html?pageNo=${pagedNews.currentPageNo+1}">${pagedNews.currentPageNo+1}</option>
						</c:if>
						<c:if
							test="${pagedNews.currentPageNo+2 <= pagedNews.totalPageCount}">
							<option
								value="${context}/manage/onesleectaccounts-${name}.html?pageNo=${pagedNews.currentPageNo+2}">${pagedNews.currentPageNo+2}</option>
						</c:if>
						<c:if
							test="${pagedNews.currentPageNo+3 <= pagedNews.totalPageCount}">
							<option
								value="${context}/manage/onesleectaccounts-${name}.html?pageNo=${pagedNews.currentPageNo+3}">${pagedNews.currentPageNo+3}</option>
						</c:if>
						<c:if
							test="${pagedNews.currentPageNo+4 <= pagedNews.totalPageCount}">
							<option
								value="${context}/manage/onesleectaccounts-${name}.html?pageNo=${pagedNews.currentPageNo+4}">${pagedNews.currentPageNo+4}</option>
						</c:if>
					</select>页
				</div>
			</div>
