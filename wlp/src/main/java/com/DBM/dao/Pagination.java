	...
	public static final int NUMBERS_PER_PAGE = 10;
	 ...
	 //当前页码
	  ...
	  public Pagination(String sql,int currentPage,int numPerPage,JdbcTemplate jTemplate){
	    ...
	    //拼接字符串用于查询总记录数（2017-4-24修改）
	    //若有错误，修改如下
	    //原来为：String totalSQL =sql.substring(0,sql.toLowerCase().indexOf("order by")).replace(sql.substring(7, sql.toLowerCase().indexOf(" from")),"count(1)");
	    String totalSQL =sql.substring(0,sql.toLowerCase().indexOf("order by")).replace(sql.substring(7, sql.toLowerCase().indexOf(" from")),"count(1)");//替换掉*，计算总数无需取出全部自动,同时去掉排序
